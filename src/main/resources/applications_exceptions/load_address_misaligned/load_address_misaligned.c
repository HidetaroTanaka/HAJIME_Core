...
uintptr_t handle_trap(uintptr_t cause, uintptr_t epc, uintptr_t regs[32]) {
  ...
  if(cause == 4) {
    printstr("SUCCESSFULLY HANDLED LOAD ADDRESS MISALIGNED EXCEPTION AT PC:");
    printstr(epchex);
    printstr("\n");
    return epc+4;
  } else {
...
int main(int argc, char** argv) {
  ...
  char* ptr_char;
  char string[19];
  printstr("START OF LOAD ADDRESS MISALIGNED TEST FOR BYTE:\n");
  for(i=0; i<8; i++) {
  ...
  short* ptr_short;
  printstr("START OF LOAD ADDRESS MISALIGNED TEST FOR HALFWORD:\n");
  for(i=0; i<8; i++) {
  ...
  int* ptr_int;
  printstr("START OF LOAD ADDRESS MISALIGNED TEST FOR WORD:\n");
  for(i=0; i<8; i++) {
  ...
  long* ptr_long;
  printstr("START OF LOAD ADDRESS MISALIGNED TEST FOR DOUBLEWORD:\n");
  for(i=0; i<8; i++) {
