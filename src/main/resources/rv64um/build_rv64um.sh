...
riscv64-unknown-elf-gcc -march=rv64im_zicsr -mabi=lp64 -static -mcmodel=medany -fvisibility=hidden -nostdlib -nostartfiles -I ../headers/ -T ../link.ld ../../../../submodule/riscv-tests/isa/rv64um/$1.S -o $1.out
riscv64-unknown-elf-objdump --disassemble-all --disassemble-zeroes --section=.text --section=.text.startup --section=.text.init --section=.data $1.out > ./$1.dump
